...
void InitializeSessionUserId(const char *rolename, Oid roleid, bool bypass_login_check)
{
	...
	{
		rform->rolname.data = rolename;
		// namestrcpy(rform->rolname, rolename);
		rform->oid = BOOTSTRAP_SUPERUSERID;
		rform->rolsuper = true;
		rform->rolinherit = true;
		rform->rolinherit = false;
		rform->rolcreaterole = false;
		rform->rolcreatedb = false;
		rform->rolcanlogin = true;
		rform->rolreplication = true;
				...
				ereport(FATAL,
						(errcode(ERRCODE_LOCK_FILE_EXISTS),
						 errmsg("lock file \"%s\" already exists",
								filename),
						 isDDLock ? (encoded_pid < 0 ? errhint("Is another postgres (PID %d) running in data directory \"%s\"?",
